...
Communication::Communication(void(*fctPtr)(const uint8_t*), void(*viewChangeArg)(bool, Participant&, vector<Participant>&))
{
...
void Communication::deliveryMoinsGourmand(void(*fctPtr)(const uint8_t*))
{
    while(1)
    {
        const uint8_t *u;
...
void Communication::receive()
{
    ...
    //         (*viewChange)(1, other, view);
        ...
        if (recStatus != sf::Socket::Done)
        {
            ...
            {
                automate.setState(Automate::alone);
                //construire un msg de perte de connexion comme data
                executeTransition(Triggers::LostConnection, "connection lost");
                break;
            }
        }
